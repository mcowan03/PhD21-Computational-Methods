...
vA = zeros(T,1);
    for t = 1:T
        vA(t) = 0.90^(t-1)*log(A_new);
    end
vA = exp(vA);


vK = K1 * ones(T+1,1); % K path guess
vK(1) = K2;
vN = N1 * ones(T,1); % N path guess
vr = r_ss1 * ones(T,1); 
vw = w_ss1 * ones(T,1);
C_ss1 = r_ss1 * K1 + w_ss1 * N1;
vC = vr.*vK(1:T) + vw.*vN; % 
...
while error1 > 1e-7
    ...
    vCprime = [vC(2:T); C_ss1]; % T length vector, corresponds to t+1. Guessed.
    vrprime = [vr(2:T); r_ss1]; % T length vector, corresponds to t+1. Guessed.
    vKprimeprime = [vK(3:T+1); K1]; % T length vector, corresponds to t+2.
