...
function [logzgrid, P] = tauchen(Ny, rho_y, sigma_y, y_bar, nStd)
...
% 1. Calculate Unconditional Standard Deviation (psi)
    ...
    psi = sigma_y / sqrt(1 - rho_y^2); 
    
    % 2. Calculate Innovation Standard Deviation (sigma_epsilon)
    % This is the standard deviation of the residual (epsilon_t)
    sigma_epsilon = sigma_y; % Assuming sigma is the standard deviation of the residual (epsilon)
    ...
    wstep = 2 * nStd * psi / (Ny - 1); % Step size (w)
    z_min = y_bar - nStd * psi;      % Minimum state
    
    logzgrid = linspace(z_min, z_min + (Ny - 1) * wstep, Ny);

    P = zeros(Ny, Ny);
    
    % 4. Calculate Transition Matrix (P)
    for i = 1:Ny % Current state x_t = Z_vals(i)
        
        % The expected mean of the next state (conditional mean)
        cond_mean = (1 - rho_y) * y_bar + rho_y * logzgrid(i);
        ...
        P(i, Ny) = 1 - normcdf(logzgrid(Ny) - wstep/2, cond_mean, sigma_epsilon);
