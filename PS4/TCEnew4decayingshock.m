...
mVF_old = results_old.mVF; % We now need this as the terminal condition
...
fprintf('\nStarting Perfect Foresight Algorithm (PFA) loop...\n');
    ...
    % A SHOCK DECAYING PATH !
    A_path = zeros(T,1);
    for t = 1:T
        A_path(t) = 0.90^(t-1)*log(A_shock);
    end
    A_path = exp(A_path);
...
while error_path > tol_pfa
    ...
    r_path = alpha * A_path .* (K_path_old / supplyL).^(alpha-1) - delta;
    w_path = (1 - alpha) * A_path .* (K_path_old / supplyL).^alpha;
    
    % --- I. Household Optimization (Backward Induction) ---
    V_path = zeros(Na, Nz, T);
    aPrimePol_path = zeros(Na, Nz, T);
    
    V_path(:, :, T) = mVF_old; % Terminal condition V_T
